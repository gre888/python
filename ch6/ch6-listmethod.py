...
lst1.append(66)
print(f"1.append(66){lst1}")
# 2. insert(index, value) - 插入指定位置 (索引 2)
lst1.insert(2, 77)
print(f"2.insert(2, 77){lst1}")
# 3. count(value) 與 index(value) - 統計與尋找位置
c=lst1.count(30)
i=lst1.index(30)
print(f"3.數字 30 出現 {c} 次，索引: {i}") 
# 4. remove(value) - 刪除第一個指定的值 (刪除 20)
lst1.remove(20)
print(f"4.remove(20) {lst1}")
# 5. pop(index) - 刪除指定位置元素並回傳 (刪除索引 3 的元素)
r=lst1.pop(3)
print(f"5.移除了 [{r}]，剩餘{lst1}") 
# 6. pop() - 預設刪除最後一個元素
lst1.pop()
print(f"6. 移除了最後 剩餘{lst1}")
...
print(f"del lst2[1:5:2] {lst2}") # [11, 33, 55, 66, 77]
# 8. clear() - 清空整個串列
lst2.clear()
